...
describe('GET /api/qrng', () => {
  ...
  describe('Edge Cases', () => {
    test('should accept maximum valid length', async () => {
      const mockAnuResponse = {
        data: { data: new Array(1024).fill(42) }
      };
    ...
    test('should accept minimum valid length', async () => {
      const mockAnuResponse = {
        data: { data: [42] }
      };
    ...
    test('should handle hex16 with size parameter', async () => {
      const mockAnuResponse = {
        data: { data: ['ABCD', 'EF01', '2345', '6789'] }
      };
      ...
      const response = await request(app)
        .get('/api/qrng')
        .query({ type: 'hex16', length: '4', size: '4' });
